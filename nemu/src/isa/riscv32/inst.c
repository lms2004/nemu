...
#define immB() do { *imm = BITS(i, 31, 31) << 12 | BITS(i, 7, 7) << 11 | (SEXT(BITS(i, 30, 25), 6) << 5)  | (SEXT(BITS(i, 11, 8), 4) << 1); } while(0)
#define immJ() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 20) | (SEXT(BITS(i, 19, 12), 8) << 12) | (BITS(i, 20, 20) << 11) | (SEXT(BITS(i, 30, 21), 10) << 1); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  /* S     imm7    rs2   rs1   func3 imm5 opcode                                          */
  /* B (imm1 imm6) rs2   rs1   func3 imm5 opcode                                          */
...
// Branch
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne   , B, if ((int)src1 != (int)src2) s->dnpc = s->pc + imm);
...
// Shift Computation
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti  , R, R(rd) = ((int)src1 < (int)imm)? 1 : 0);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , R, R(rd) = ((word_t)src1 < (word_t)imm)? 1 : 0);
...
// load and store
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu   , I, R(rd) = Mr((word_t)src1 + imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu   , I, R(rd) = Mr((word_t)src1 + imm, 2));
