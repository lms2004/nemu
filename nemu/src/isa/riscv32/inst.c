...
// Shift Computation
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , R, R(rd) = ((uint32_t)src1 < imm)? 1 : 0);
...
// load and store
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu   , I, R(rd) = Mr((uint32_t)src1 + imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu   , I, R(rd) = Mr((uint32_t)src1 + imm, 2));
